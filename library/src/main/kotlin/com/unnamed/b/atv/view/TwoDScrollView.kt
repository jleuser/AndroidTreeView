...
class TwoDScrollView : FrameLayout {
       ...
       */
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (deltaX < 0) {
                    ...
                    deltaX = when {
                        availableToScroll > 0 -> Math.min(availableToScroll, deltaX)
                        else -> 0
                    }
                }
                if (deltaY < 0) {
                    ...
                    deltaY = when {
                        availableToScroll > 0 -> Math.min(availableToScroll, deltaY)
                        else -> 0
                    }
                }
   ...
   */
        ...
        return if (preferredFocusable != null
            ...
            preferredFocusable
        } else findFocusableViewInBounds(topFocus, topWithoutFadingEdge, bottomWithoutFadingEdge, leftFocus, leftWithoutFadingEdge, rightWithoutFadingEdge)
    }
     ...
     *
     * @param directionY the scroll direction: [View.FOCUS_UP]
    ...
    protected fun computeScrollDeltaToGetChildRectOnScreen(rect: Rect): Int {
        ...
        if (rect.bottom > screenBottom && rect.top > screenTop) {
            ...
            // screen size chunk).
            scrollYDelta += when {
                rect.height() > height -> // just enough to get screen size chunk on
                    rect.top - screenTop
            ...
            scrollYDelta -= when {
                rect.height() > height -> // screen size chunk
    ...
    override fun onRequestFocusInDescendants(direction: Int, previouslyFocusedRect: Rect?): Boolean {
        ...
        // (ugh).
        when (dir) {
            View.FOCUS_FORWARD -> dir = View.FOCUS_DOWN
            View.FOCUS_BACKWARD -> dir = View.FOCUS_UP
        }
        ...
        return nextFocus.requestFocus(dir, previouslyFocusedRect)
    }
    ...
    fun fling(velocityX: Int, velocityY: Int) {
        if (childCount > 0) {
            ...
            var newFocused = findFocusableViewInMyBounds(movingRight, mScroller!!.finalX, movingDown, mScroller!!.finalY, findFocus())
            if (newFocused == null) {
                newFocused = this
            }
    ...
    override fun scrollTo(x: Int, y: Int) {
        ...
        if (childCount > 0) {
            val child = getChildAt(0)
            tmpX = clamp(tmpX, width - paddingRight - paddingLeft, child.width)
            tmpY = clamp(tmpY, height - paddingBottom - paddingTop, child.height)
            if (tmpX != scrollX || tmpY != scrollY) {
                super.scrollTo(tmpX, tmpY)
            }
        }
    }
